...
- [x] **Fase 3b — Import de PDF de exame**: upload do PDF na aba Exames → `ExamPdfExtractor` (API Anthropic, leitura de PDF, modelo fixo `claude-sonnet-4-6`) extrai os resultados → cria `measurements` (source "pdf", idempotente). Requer `ANTHROPIC_API_KEY` no ambiente; sem a chave a UI mostra aviso e o endpoint recusa. PDF processado em memória (não persistido). *(Concluída 2026-06-29.)*
...
**Status: Fases 1–6 concluídas e em produção (2026-06-29).** Pendência operacional: definir `ANTHROPIC_API_KEY` no Railway para ligar o import de PDF de exames (Fase 3b). Modelo de extração fixo: `claude-sonnet-4-6`.
